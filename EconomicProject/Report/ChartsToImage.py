...
def npv_to_image(workbook, path='Charts/'):
    # График сроков окупаемости
    column = 4
    worksheet = workbook["NPV"]
    data = {
...
def elastic_to_image(workbook, path='Charts/'):
    # График общей чувствительности
    column = 3
    worksheet = workbook["Elastic"]
    data = {
...
def montecarlo_to_image(workbook, path='Charts/'):
    worksheet = workbook['MonteСarlo']
    data = {
...
def charts_to_image(workbook):
    npv_to_image(workbook)
    elastic_to_image(workbook)
    montecarlo_to_image(workbook)


excel_file = "Results/Record_Report_E3.xlsx"
